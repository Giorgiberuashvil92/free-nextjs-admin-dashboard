...
export default function ServicesAdminPage() {
  ...
  const handleVipToggle = async (service: Service) => {
    ...
    setError("");
    try {
      await apiPatch(`/services/${id}`, { isFeatured: !service.isFeatured });
      await load();
    } catch (e: unknown) {
      const message =
        e && typeof e === "object" && "message" in e
          ? String((e as { message?: unknown }).message)
          : "VIP სტატუსის შეცვლა ვერ მოხერხდა";
      setError(message);
    } finally {
            ...
            <div key={service.id} className="bg-white border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="p-4">
                ...
                <div className="flex gap-2 mt-4">
                  <button
                    onClick={() => handleVipToggle(service)}
                    disabled={vipUpdatingId === (service.id || service._id)}
                  ...
                  >
                    {vipUpdatingId === (service.id || service._id)
                      ? "იტვირთება..."
                      : service.isFeatured
                        ? "VIP-ის მოხსნა"
                        : "VIP-ზე აყვანა"}
