...
export default function DetailingListPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiGetJson<{ success: boolean; data: Store[] } | Store[]>(`/detailing${qs}`);
      ...
      const storesWithStats = await Promise.all(
        stores.map(async (store) => {
          ...
          try {
            const endpoints = [
              `/detailing/${store.id}/stats`,
              `/detailing/${store.id}/engagement`,
  ...
  const loadEngagement = useCallback(async (storeId: string) => {
    ...
    try {
      const endpoints = [
        `/detailing/${storeId}/engagement`,
        `/detailing/${storeId}/likes`,
        `/detailing/${storeId}/views`,
        `/detailing/${storeId}/calls`,
      ...
      if (!engagementData.likes) {
        try {
          const likesRes = await fetch(`${API_BASE}/detailing/${storeId}/likes`, { cache: "no-store" });
          if (likesRes.ok) {
      ...
      if (!engagementData.views) {
        try {
          const viewsRes = await fetch(`${API_BASE}/detailing/${storeId}/views`, { cache: "no-store" });
          if (viewsRes.ok) {
      ...
      if (!engagementData.calls) {
        try {
          const callsRes = await fetch(`${API_BASE}/detailing/${storeId}/calls`, { cache: "no-store" });
          if (callsRes.ok) {
  ...
  const handleDelete = async (store: Store, e: React.MouseEvent) => {
    ...
    try {
      await apiDelete(`/detailing/${store.id}`);
      load(); // Reload list
    } catch (error) {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-semibold">დითეილინგი</h1>
        ...
        <div className="flex items-end gap-3 flex-wrap">
          ...
          <Link className="px-3 py-2 border rounded inline-block" href="/detailing/new">
            New detailing store
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {loading ? (
          ...
          <div className="col-span-full text-center text-gray-500 py-10">No detailing stores</div>
        ) : (
          rows.map((s) => {
